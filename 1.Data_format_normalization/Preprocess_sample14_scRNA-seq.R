...
sample14_rna <- sce[,sample14_cells]
sample14_medadata <- metdata_list %>% filter(cells %in% sample14_cells) %>% as.data.frame()

rownames(sample14_medadata) <- sample14_medadata$cells

sample14_medadata <- sample14_medadata[colnames(sample14_rna),]

identical(colnames(sample14_rna),rownames(sample14_medadata))

sample14_rna <- sample14_rna %>% AddMetaData(metadata = sample14_medadata)

sample14_rna$cell_type <- sample14_rna$lineage
