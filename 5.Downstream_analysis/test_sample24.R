...
source('plot.R')
### path need to define
output_path = 'E:\\DRCTdb\\ignore\\downstream_result\\sample24\\'
rna_path = 'F:\\DRCTdb\\ignore\\scRNA-seq\\Sample24\\Sample24_exp_matrix.Rds'
atac_path = 'E:\\DRCTdb\\ignore\\bed\\sample24/'
ldsc_path = "E:/DRCTdb/ignore/LDSC_results/sample24/pvalues.tsv"
#### db path
...
meta=read.csv("F:/DRCTdb/ignore/scRNA-seq/Sample24/GSE214979_cell_metadata.csv.gz", row.names=1)
mt1 = rna@assays@data@listData$counts
colnames(mt1) = gsub('Sample24#','',colnames(mt1))
rna2 = CreateSeuratObject(mt1,meta.data = meta)
rna2@active.ident=as.factor(rna2$subs)
